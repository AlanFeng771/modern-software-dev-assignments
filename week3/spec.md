...
- **Auth (bonus): pre-shared API key, opt-in via env var, HTTP-only**: chose the assignment's simpler "API key" bonus option over full OAuth2 with audience validation (no external IdP, no token issuance/expiry to build). Implemented via the SDK's `TokenVerifier` hook (`week3/server/api_key_auth.py`): `verify_token` compares the bearer token to `MCP_API_KEY` and returns `None` on mismatch, which the SDK turns into a `401` automatically. Enabled only when `MCP_API_KEY` is set (`MCPServer(auth=..., token_verifier=...)` requires both together, or a `ValueError` at construction) — this keeps STDIO/local/test usage unauthenticated by default without a separate code path. Confirmed by reading the SDK source that `run_stdio_async` and the `initialize` handshake never read `settings.auth` — only `streamable_http_app`/`sse_app` do — so this setting has zero effect on STDIO regardless of whether the env var happens to be set.
